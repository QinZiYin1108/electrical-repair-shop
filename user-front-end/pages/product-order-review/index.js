...
const { fetchUserProductOrderDetail } = require('../../api/userProductOrders');
const {
...
const TEXTS = {
  ...
  title: '商品评价',
  viewOrder: '查看订单',
  goodsTitleLabel: '商品信息',
  orderNoLabel: '订单号',
  itemCountLabel: '商品件数',
  statusLabel: '订单状态',
  completedLabel: '完成时间',
  formTitle: '请为本次商品体验打分',
  scoreTitle: '商品评分',
  contentTitle: '评价内容',
  contentPlaceholder: '可以描述商品质量、外观包装、使用体验等，帮助其他用户做出选择',
  imageTitle: '评价图片',
  imageTip: '最多上传 5 张图片，提交时统一上传',
  anonymousLabel: '匿名评价',
  anonymousTip: '开启后，其他用户看到的将是匿名用户',
  addImage: '添加图片',
  submit: '提交评价',
  submitting: '提交中...',
...
function mapOrderDetail(detail) {
  ...
  return {
    ...
    productSummary: item.productSummary || '商品订单',
    itemCount: Number(item.itemCount || 0),
    orderStatusText: item.orderStatusText || '订单状态',
    deliveryStatusText: item.deliveryStatusText || '',
    paymentStatusText: item.paymentStatusText || '',
    completionTimeText: formatDateTime(item.completionTime || item.receiveTime || item.updatedTime || 0),
    canReview: !!item.canReview,
    hasReview: !!item.hasReview,
    items: (Array.isArray(item.items) ? item.items : []).map((goods) => ({
      id: goods.id || '',
      productId: goods.productId || '',
      productName: goods.productName || '商品信息待补充',
      productImage: goods.productImage || '',
      productPriceText: formatMoney(goods.productPrice),
      totalPriceText: formatMoney(goods.totalPrice),
...
function createLocalImageItem(file, index) {
  ...
  return {
    id: `product-review-image-${Date.now()}-${index}`,
    url: path,
    thumbnailUrl: path,
    localPath: path
  };
...
function createUploadedImagePayload(item, uploaded, index) {
  return {
    url: uploaded.url || '',
    name: uploaded.name || `product-review-image-${index + 1}.jpg`,
...
Page({
  ...
  loadPage() {
    ...
    Promise.all([
      ...
      fetchUserProductOrderReview(this.data.orderId)
    ])
  ...
  onViewOrderTap() {
    ...
    router.navigateTo({
      url: `/pages/product-order-detail/index?orderId=${detail.id}`
    });
  },
  ...
  onSubmitTap() {
    ...
    this.uploadPendingImages()
      .then((images) => submitUserProductReview({
        orderId: detail.id,
