...
function parseInline(text) {
  ...
  html = html.replace(/`([^`]+)`/g, (_, code) => {
    return `<code style="padding: 2px 6px; border-radius: 6px; background: #f3f6fb; color: #b42318; font-family: monospace; font-size: 0.92em;">${code}</code>`;
  });
...
function renderList(items, ordered) {
  ...
  const padding = ordered ? 'padding-left: 24px;' : 'padding-left: 22px;';
  return `<${tag} style="margin: 0 0 16px; ${padding} color: #243c4e; line-height: 1.9;">${items.map((item) => `<li style="margin: 8px 0;">${parseInline(item)}</li>`).join('')}</${tag}>`;
}
